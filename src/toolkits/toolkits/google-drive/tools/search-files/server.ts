...
> => {
  return {
    callback: async ({ 
      ...
      const folderCache = new Map<string, { name: string; parents?: string[] }>();
      ...
      // Build search query
      const buildSearchQuery = (query: string, options: {
        mimeType?: string;
        folderId?: string;
        nameOnly?: boolean;
        ...
        if (query.trim()) {
          ...
          if (options.nameOnly) {
            ...
            conditions.push(`(name contains '${cleanQuery}' or fullText contains '${cleanQuery}')`);
          }
        }
        ...
        if (options.fileTypes && options.fileTypes.length > 0) {
          ...
          if (mimeTypes.length > 0) {
            const mimeConditions = mimeTypes.map(mt => `mimeType='${mt}'`).join(' or ');
      ...
      const transformedFiles = await Promise.all(
        finalFiles.map(async (file) => ({
          ...
          path: await buildFilePath(file.parents),
        }))
      );
      ...
      return {
        ...
        incompleteSearch: folderIds.length > 0 && processedResults >= maxResults,
        searchStats: {
